        ...
        for (int i = 0; i < 3; i++) {
            ...
            try {
                if (i == 1) {
                    ...
                    // resume thread for invokedMethod to have suspend count equal to 1.
                    thread.resume();
                }
                display("invoking the method");
                retValue = testedClass.invokeMethod(thread, method, params, 0);

                long resultTime = ((PrimitiveValue)retValue).longValue();
                if ( resultTime >= waitTime) {
                ...
                complain("Unexpected exception while invocation of debugee's method: " + e);
                exitStatus = Consts.TEST_FAILED;
            }
        }
