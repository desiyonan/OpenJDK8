...
public class compareto001 {
    ...
    private final static String prefix = "nsk.jdi.FloatValue.compareTo";
    ...
    private static void execTest() {
        ...
        for (int i = 0; i < fieldList.size(); i++ ) {
            ...
            display("Checking compateTo(Object object) method for FloatValue: " + value);
    ...
    private static boolean PerformComparing(FloatValue value, Object object ) {
        ...
        if (object instanceof FloatValue) {
            FloatValue floatObject = (FloatValue)object;
            try {
                // assertion [ x.compareTo(y) == 0 <==> x.equals(y) ]
                if ( ((value.equals(object)) && (value.compareTo(floatObject) != 0)) ||
                     (!(value.equals(object)) && (value.compareTo(floatObject) == 0)) ) {
                ...
                if ( ((value.compareTo(floatObject) == 0) && (floatObject.compareTo(value) != 0)) ||
                     ((value.compareTo(floatObject) != 0) && (floatObject.compareTo(value) == 0)) ) {
                    complain("Failed assertion [ (x.compareTo(y) == 0) is identical to (y.compareTo(x) == 0) ] \n\t" +
                             "where 'x' is FloatValue: " + value + " and 'y' is FloatValue : " + floatObject + " \n\t" +
                             "result of (x.compareTo(y)): " + value.compareTo(floatObject) + "\n\t" +
                             "result of (y.compareTo(x)): " + floatObject.compareTo(value) );
                    result = false;
                }
                if (value.compareTo(floatObject) != 0) {
                    // assertion [ if (x.compareTo(y) == i) then (y.compareTo(x) == -i) ]
                    if (value.compareTo(floatObject) != -(floatObject.compareTo(value))) {
                ...
                if (value.compareTo(floatObject) > 0) {
                     FloatValue lessValue = FindLessFloatValue(floatObject);
                     if (lessValue != null) {
                ...
                complain("Caught unexpected " + e + " when comparing \n\t" +
                         "FloatValue: " + value + " and FloatValue argument: " + object);
                result = false;
            }
            ...
            try {
                ...
                complain("Does not throw expected NullPointerException when comparing \n\t" +
                         "FloatValue: " + value + " and null argument");
                result = false;
            ...
            try {
                value.compareTo((FloatValue)object);
                complain("Does not throw expected ClassCastException when comparing \n\t" +
                         "FloatValue: " + value + " and argument: " + object);
                result = false;
     ...
     *  to find another FloatValue field which is less then method's argument via
     *  <i>compareTo</i> method.
     */

    private static FloatValue FindLessFloatValue (FloatValue currentValue) {
        FloatValue result = null;

        for (int i = 0; i < fieldList.size(); i++ ) {

            Field field = (Field )fieldList.get(i);
            FloatValue newValue = (FloatValue)((ObjectReference )objectValue).getValue(field);
