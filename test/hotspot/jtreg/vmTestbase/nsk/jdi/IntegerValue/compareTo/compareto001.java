...
public class compareto001 {
    ...
    private final static String prefix = "nsk.jdi.IntegerValue.compareTo";
    ...
    private static void execTest() {
        ...
        for (int i = 0; i < fieldList.size(); i++ ) {
            ...
            display("Checking compateTo(Object object) method for IntegerValue: " + value);
    ...
    private static boolean PerformComparing(IntegerValue value, Object object ) {
        ...
        if (object instanceof IntegerValue) {
            IntegerValue intObject = (IntegerValue)object;
            try {
                // assertion [ x.compareTo(y) == 0 <==> x.equals(y) ]
                if ( ((value.equals(object)) && (value.compareTo(intObject) != 0)) ||
                     (!(value.equals(object)) && (value.compareTo(intObject) == 0)) ) {
                ...
                if ( ((value.compareTo(intObject) == 0) && (intObject.compareTo(value) != 0)) ||
                     ((value.compareTo(intObject) != 0) && (intObject.compareTo(value) == 0)) ) {
                    complain("Failed assertion [ (x.compareTo(y) == 0) is identical to (y.compareTo(x) == 0) ] \n\t" +
                             "where 'x' is IntegerValue: " + value + " and 'y' is IntegerValue : " + intObject + " \n\t" +
                             "result of (x.compareTo(y)): " + value.compareTo(intObject) + "\n\t" +
                             "result of (y.compareTo(x)): " + intObject.compareTo(value) );
                    result = false;
                }
                if (value.compareTo(intObject) != 0) {
                    // assertion [ if (x.compareTo(y) == i) then (y.compareTo(x) == -i) ]
                    if (value.compareTo(intObject) != -(intObject.compareTo(value))) {
                ...
                if (value.compareTo(intObject) > 0) {
                     IntegerValue lessValue = FindLessIntegerValue(intObject);
                     if (lessValue != null) {
                ...
                complain("Caught unexpected " + e + " when comparing \n\t" +
                         "IntegerValue: " + value + " and IntegerValue argument: " + object);
                result = false;
            }
            ...
            try {
                ...
                complain("Does not throw expected NullPointerException when comparing \n\t" +
                         "IntegerValue: " + value + " and null argument");
                result = false;
            ...
            try {
                value.compareTo((IntegerValue)object);
                complain("Does not throw expected ClassCastException when comparing \n\t" +
                         "IntegerValue: " + value + " and argument: " + object);
                result = false;
     ...
     *  to find another IntegerValue field which is less then method's argument via
     *  <i>compareTo</i> method.
     */

    private static IntegerValue FindLessIntegerValue (IntegerValue currentValue) {
        IntegerValue result = null;

        for (int i = 0; i < fieldList.size(); i++ ) {

            Field field = (Field )fieldList.get(i);
            IntegerValue newValue = (IntegerValue)((ObjectReference )objectValue).getValue(field);
