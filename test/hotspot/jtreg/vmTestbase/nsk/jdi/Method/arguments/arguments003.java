...
public class arguments003 {
    ...
    private final static String prefix = "nsk.jdi.Method.arguments.";
    private final static String className = "arguments003";
    ...
    private static void execTest() {
        ...
        for (int i = 0; i < expSignatures.length; i++) {
            ...
            if (method != null) {
                try {
                    Iterator<LocalVariable> arguments = method.arguments().iterator();
                    boolean found = false;
                    while (arguments.hasNext()) {
                        LocalVariable argument = arguments.next();
                        if (argument.typeName().equals(expSignatures[i][1])) {
                            found = true;
                        }
                    }
                    if (found) {
                        display("Method.arguments() returned expected varargs argument of type: " + expSignatures[i][1]);
                    } else {
                        failure("Method.arguments() did not return expected varargsvarargs argument of type: " + expSignatures[i][1]);
                    }
