 ...
 * properly throws <i>InvalidTypeException</i> when
 * a debugger part of the test invokes several debuggee methods
 * with different kinds of arguments which are not assignment
 * compatible with the argument type and not convertible without
...
public class invokemethod007 {
    ...
    static final String DEBUGGEE_LOCALVAR =
        "invokemethod007tdummyCls";

    // tested debuggee methods and fields used only to obtian their values
    static final int METH_NUM = 19;
    static final String DEBUGGEE_METHODS[][] = {
        {"byteMeth", "sStrFld"},
        {"shortMeth", "intFld"},
        {"intMeth", "sLongFld"},
        {"longMeth", "floatFld"},
        {"floatMeth", "sDoubleFld"},
        {"doubleMeth", "sBooleanFld"},
        {"charMeth", "sStrFld"},
        {"booleanMeth", "sByteFld"},
        {"strMeth", "sCharFld"},
        {"dummyTMeth", "sByteFld"},
        {"dummyTMeth", "sShortFld"},
        {"dummyTMeth", "intFld"},
        {"dummyTMeth", "sLongFld"},
        {"dummyTMeth", "floatFld"},
        {"dummyTMeth", "sDoubleFld"},
        {"dummyTMeth", "sCharFld"},
        {"dummyTMeth", "sBooleanFld"},
        {"dummyTMeth", "sStrFld"},
        {"dummyTMeth", "wrongDummyType"}
    };
    ...
    public static int run(String argv[], PrintStream out) {
        return new invokemethod007().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "invokemethod007t.err> ");
        debuggee.resume();
        cmd = pipe.readln();
...
// Check the tested assersion
        try {
            ...
            for (int i=0; i<METH_NUM; i++) {
                List methList = rType[1].methodsByName(DEBUGGEE_METHODS[i][0]);
                if (methList.isEmpty()) {
                ...
                argList.add(objRef.getValue(
                    rType[1].fieldByName(DEBUGGEE_METHODS[i][1])));
                ...
                try {
                    log.display("\nTrying to invoke the method \""
                        ...
                        + "\"\n\twith the arguments: "
                        + rType[1].fieldByName(DEBUGGEE_METHODS[i][1]) + " " + argList
                    ...
                    // Make sure that the reference type is really loaded by the debuggee VM
