 ...
 * <code>com.sun.jdi.ReferenceType.allFields()</code>
 * properly returns an empty field list for arrays (ArrayType) and primitive
 * classes.<p>
 *
 * Debugger part of it attempts to get a list of all fields declared
 * and inherited in several reference types corresponding to debuggee field
 * values, which themselves are:<br>
 * primitive classes, and arrays of primitive types and classes.
 */
public class allfields005 {
    ...
    // tested allfields used to verify the assertion
    ...
    public static int run(String argv[], PrintStream out) {
        return new allfields005().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "allfields005t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                try {
                    log.display((i+1) + ") Trying to get a list containing all fields declared and inherited in the reference type \""
                        + testedType + "\"\tname " + testedType.name()
                        + " ...");
                    List meths = testedType.allFields();
                    if (meths.size() == 0)
                        log.display("CHECK PASSED: returned list is empty as expected");
                    else {
                        log.complain("TEST FAILED: returned list containing all fields declared and inherited in the reference type \""
                            + testedType + "\"\tname: " + testedType.name()
                    ...
                    log.complain("TEST FAILED: caught unexpected "
                        + ue + "\n\tinstead of an empty list of all fields"
                        + "\n\tdeclared and inherited in the reference type \""
                        + testedType + "\"\tname " + testedType.name());
                    tot_res = Consts.TEST_FAILED;
                }
