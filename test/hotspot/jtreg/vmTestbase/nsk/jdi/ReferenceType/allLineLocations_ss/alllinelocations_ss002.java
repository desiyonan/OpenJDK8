 ...
 * <code>com.sun.jdi.ReferenceType.allLineLocations(String,String)</code>
 ...
 * an interface type.
 */
public class alllinelocations_ss002 {
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "alllinelocations_ss002t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                for (int j=0; j<2; j++) {
                    ...
                    try {
                        log.display((i+1) + "." + (j+1)
                            + ") Trying to get all locations that map to the reference type \""
                            + testedType + "\"\tname: " + testedType.name()
                            + "\n\tusing " + testCase + " as a stratum parameter ...");
                        switch(j) {
                        case 0:
                            locs = testedType.allLineLocations(null, null);
                            break;
                        case 1:
                            locs = testedType.allLineLocations(
                                vm.getDefaultStratum(), null);
                            break;
                        }
                        if (locs.size() == 0)
                            log.display("CHECK PASSED: returned list is empty as expected");
                        else {
                            log.complain("TEST FAILED: returned list of all locations of the reference type \""
                                + testedType + "\"\tname " + testedType.name()
                                + "\n\twith " + testCase + " used as a stratum parameter,"
                        ...
                        log.complain("TEST FAILED: caught unexpected "
                            + ue + "\n\tinstead of an empty list of all locations of the reference type \""
                            + testedType + "\"\tname " + testedType.name()
                            + "\n\twith " + testCase + " used as a stratum parameter");
                        tot_res = Consts.TEST_FAILED;
                    }
