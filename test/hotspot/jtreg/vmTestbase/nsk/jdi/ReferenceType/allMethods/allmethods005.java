 ...
 * <code>com.sun.jdi.ReferenceType.allMethod()</code>
 * properly returns an empty list for arrays (ArrayType) and primitive
 * classes.<p>
 *
 * Debugger part of it attempts to get a list of all methods in several
...
public class allmethods005 {
    ...
    public static int run(String argv[], PrintStream out) {
        return new allmethods005().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "allmethods005t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                try {
                    log.display((i+1) + ") Trying to get a list of all methods declared in the reference type \""
                        + testedType + "\"\tname " + testedType.name()
                        + " ...");
                    List meths = testedType.allMethods();
                    if (meths.size() == 0)
                        log.display("CHECK PASSED: returned list is empty as expected");
                    else {
                        log.complain("TEST FAILED: returned list of all methods declared and inherited in the reference type \""
                            + testedType + "\"\tname: " + testedType.name()
                    ...
                    log.complain("TEST FAILED: caught unexpected "
                        + ue + "\n\tinstead of an empty list of all methods"
