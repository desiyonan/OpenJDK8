...
public class hashcode002 {
    ...
    private final static String
        ...
        thisClassName = package_prefix + "hashcode002",
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new hashcode002().runThis(argv,out);
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        print_log_anyway("==> nsk/jdi/ReferenceType/hashCode/hashcode002 test LOG:");
        print_log_anyway("--> test checks hashCode() method of ReferenceType interface ");
        ...
        print_log_on_verbose("--> hashcode002: hashcode002a debugee launched");
        debugee.resume();

        String debugee_signal = pipe.readln();
        if (debugee_signal == null) {
            print_log_anyway
                ("##> hashcode002: UNEXPECTED debugee's signal (not \"ready0\") - " + debugee_signal);
        ...
        if (!debugee_signal.equals("ready0")) {
            ...
            print_log_on_verbose("--> hashcode002: debugee's \"ready0\" signal recieved!");
        }
        ...
        if (debugee_signal == null) {
            print_log_anyway
                ("##> hashcode002: UNEXPECTED debugee's signal (not \"ready1\") - " + debugee_signal);
            return 2/*STATUS_FAILED*/;
        }
        if (!debugee_signal.equals("ready1")) {
            ...
            print_log_on_verbose("--> hashcode002: debugee's \"ready1\" signal recieved!");
        }

        boolean class_not_found_error = false;
        boolean hashCode_method_error = false;
        while ( true ) {
            print_log_on_verbose
                ("--> hashcode002: getting ReferenceType object for loaded checked class...");
            ReferenceType refType = debugee.classByName(checked_class);
            if (refType == null) {
                print_log_without_verbose
                    ("--> hashcode002: getting ReferenceType object for loaded checked class...");
                print_log_anyway("##> hashcode002: FAILED: Could NOT FIND checked class: " + checked_class);
                class_not_found_error = true;
                break;
            }
            else {
                print_log_on_verbose("--> hashcode002: checked class FOUND: " + checked_class);
            }
            print_log_on_verbose
                ("--> hashcode002: waiting for \"ready2\" or \"not_unloaded\" signal from debugee...");
            pipe.println("continue");
            debugee_signal = pipe.readln();
            if (debugee_signal == null) {
                print_log_anyway
                    ("##> hashcode002: UNEXPECTED debugee's signal - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            if ( debugee_signal.equals("not_unloaded")) {
                print_log_anyway
                    ("--> hashcode002: debugee's \"not_unloaded\" signal recieved!");
                print_log_without_verbose
                    ("-->              checked class may be NOT unloaded!");
                print_log_anyway
                    ("-->              ReferenceType.hashCode() method can NOT be checked!");
                break;
            }
            if (!debugee_signal.equals("ready2")) {
                print_log_anyway
                    ("##> hashcode002: UNEXPECTED debugee's signal (not \"ready2\") - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            else {
                print_log_on_verbose("--> hashcode002: debugee's \"ready2\" signal recieved!");
            }
            print_log_on_verbose
                ("--> hashcode002: check that checked class has been unloaded really...");
            ReferenceType refType2 = debugee.classByName(checked_class);
            if (refType2 == null) {
                print_log_on_verbose
                    ("--> hashcode002: checked class has been unloaded really: " + checked_class);
            }
            else {
                print_log_without_verbose
                    ("--> hashcode002: check that checked class has been unloaded really...");
                print_log_anyway
                    ("--> hashcode002: checked class FOUND: " + checked_class
            ...
            print_log_anyway
                ("--> hashcode002: check ReferenceType.hashCode() method for unloaded class...");
            int hash_code = 0;
            try {
                hash_code = refType.hashCode();
            }
            catch (Exception expt) {
                if (expt instanceof com.sun.jdi.ObjectCollectedException) {
                    print_log_anyway
                        ("--> hashcode002: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    print_log_anyway
                        ("##> hashcode002: FAILED: unexpected Exception thrown - " + expt.toString());
                    print_log_anyway
                        ("##>              expected Exception - com.sun.jdi.ObjectCollectedException");
                    hashCode_method_error = true;
                }
            }
            break;
        }
        int v_test_result = 0/*STATUS_PASSED*/;
        if ( class_not_found_error || hashCode_method_error ) {
            v_test_result = 2/*STATUS_FAILED*/;
        }

        print_log_on_verbose("--> hashcode002: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            print_log_anyway
                ("##> hashcode002: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> hashcode002: expected Debugee's exit status - " + status);
        }

        return v_test_result;
    }
}  // end of hashcode002 class
