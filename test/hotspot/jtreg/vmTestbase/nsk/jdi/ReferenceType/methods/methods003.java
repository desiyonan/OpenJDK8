...
public class methods003 {
    ...
    private final static String
        ...
        thisClassName = package_prefix + "methods003",
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new methods003().runThis(argv,out);
        if ( v_test_result == 2/*STATUS_FAILED*/ ) {
            print_log_anyway
                ("\n==> nsk/jdi/ReferenceType/methods/methods003 test FAILED");
        }
        else {
            print_log_anyway
                ("\n==> nsk/jdi/ReferenceType/methods/methods003 test PASSED");
        }
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        print_log_anyway("==> nsk/jdi/ReferenceType/methods/methods003 test LOG:");
        ...
        print_log_on_verbose("--> methods003: methods003a debugee launched");
        debugee.resume();

        String debugee_signal = pipe.readln();
        if (debugee_signal == null) {
            print_log_anyway
                ("##> methods003: UNEXPECTED debugee's signal (not \"ready0\") - " + debugee_signal);
        ...
        if (!debugee_signal.equals("ready0")) {
            ...
            print_log_on_verbose("--> methods003: debugee's \"ready0\" signal recieved!");
        }
        ...
        if (debugee_signal == null) {
            print_log_anyway
                ("##> methods003: UNEXPECTED debugee's signal (not \"ready1\") - " + debugee_signal);
            return 2/*STATUS_FAILED*/;
        }
        if (!debugee_signal.equals("ready1")) {
            ...
            print_log_on_verbose("--> methods003: debugee's \"ready1\" signal recieved!");
        }
        ...
        while ( true ) {
            print_log_on_verbose
                ("--> methods003: getting ReferenceType object for loaded checked class...");
            ReferenceType refType = debugee.classByName(checked_class);
            if (refType == null) {
                print_log_without_verbose
                    ("--> methods003: getting ReferenceType object for loaded checked class...");
                print_log_anyway("##> methods003: FAILED: Could NOT FIND checked class: " + checked_class);
                class_not_found_error = true;
                break;
            }
            else {
                print_log_on_verbose("--> methods003: checked class FOUND: " + checked_class);
            }
            print_log_on_verbose
                ("--> methods003: waiting for \"ready2\" or \"not_unloaded\" signal from debugee...");
            pipe.println("continue");
            debugee_signal = pipe.readln();
            if (debugee_signal == null) {
                print_log_anyway
                    ("##> methods003: UNEXPECTED debugee's signal - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            if ( debugee_signal.equals("not_unloaded")) {
                print_log_anyway
                    ("--> methods003: debugee's \"not_unloaded\" signal recieved!");
                print_log_without_verbose
                    ("-->             checked class may be NOT unloaded!");
                print_log_anyway
                    ("-->             ReferenceType.methods() method can NOT be checked!");
                break;
            }
            if (!debugee_signal.equals("ready2")) {
                print_log_anyway
                    ("##> methods003: UNEXPECTED debugee's signal (not \"ready2\") - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            else {
                print_log_on_verbose("--> methods003: debugee's \"ready2\" signal recieved!");
            }
            print_log_on_verbose
                ("--> methods003: check that checked class has been unloaded realy...");
            ReferenceType refType2 = debugee.classByName(checked_class);
            if (refType2 == null) {
                print_log_on_verbose
                    ("--> methods003: checked class has been unloaded realy: " + checked_class);
            }
            else {
                print_log_without_verbose
                    ("--> methods003: check that checked class has been unloaded realy...");
                print_log_anyway
                    ("--> methods003: checked class FOUND: " + checked_class
            ...
            print_log_anyway
                ("--> methods003: check ReferenceType.methods() method for unloaded class...");
            ...
            try {
                ...
                if (expt instanceof com.sun.jdi.ObjectCollectedException) {
                    print_log_anyway
                        ("--> methods003: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    print_log_anyway
                        ("##> methods003: FAILED: unexpected Exception thrown - " + expt.toString());
                    print_log_anyway
        ...
        print_log_on_verbose("--> methods003: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            print_log_anyway
                ("##> methods003: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> methods003: expected Debugee's exit status - " + status);
        }

        return v_test_result;
    }
}  // end of methods003 class
