...
//        package_prefix = "",    //  for DEBUG without package
        thisClassName = package_prefix + "methbyname_ss003",
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new methbyname_ss003().runThis(argv,out);
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        print_log_anyway("==> nsk/jdi/ReferenceType/methodsByName_ss/methbyname_ss003 test LOG:");
        print_log_anyway("--> test checks methodsByName(String name, String signature) method of ReferenceType ");
        print_log_anyway("    interface of the com.sun.jdi package for UNLOADED class\n");
        ...
        print_log_on_verbose("--> methbyname_ss003: methbyname_ss003a debugee launched");
        debugee.resume();

        String debugee_signal = pipe.readln();
        if (debugee_signal == null) {
            print_log_anyway
                ("##> methbyname_ss003: UNEXPECTED debugee's signal (not \"ready0\") - " + debugee_signal);
        ...
        if (!debugee_signal.equals("ready0")) {
            ...
            print_log_on_verbose("--> methbyname_ss003: debugee's \"ready0\" signal recieved!");
        }
        ...
        if (debugee_signal == null) {
            print_log_anyway
                ("##> methbyname_ss003: UNEXPECTED debugee's signal (not \"ready1\") - " + debugee_signal);
            return 2/*STATUS_FAILED*/;
        }
        if (!debugee_signal.equals("ready1")) {
            ...
            print_log_on_verbose("--> methbyname_ss003: debugee's \"ready1\" signal recieved!");
        }

        boolean class_not_found_error = false;
        boolean methodsByName_ss_method_error = false;
        while ( true ) {
            print_log_on_verbose
                ("--> methbyname_ss003: getting ReferenceType object for loaded checked class...");
            ReferenceType refType = debugee.classByName(checked_class);
            if (refType == null) {
                print_log_without_verbose
                    ("--> methbyname_ss003: getting ReferenceType object for loaded checked class...");
                print_log_anyway("##> methbyname_ss003: FAILED: Could NOT FIND checked class: " + checked_class);
                class_not_found_error = true;
                break;
            }
            else {
                print_log_on_verbose("--> methbyname_ss003: checked class FOUND: " + checked_class);
            }
            print_log_on_verbose
                ("--> methbyname_ss003: waiting for \"ready2\" or \"not_unloaded\" signal from debugee...");
            pipe.println("continue");
            debugee_signal = pipe.readln();
            if (debugee_signal == null) {
                print_log_anyway
                    ("##> methbyname_ss003: UNEXPECTED debugee's signal - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            if ( debugee_signal.equals("not_unloaded")) {
                print_log_anyway
                    ("--> methbyname_ss003: debugee's \"not_unloaded\" signal recieved!");
                print_log_without_verbose
                    ("-->                  checked class may be NOT unloaded!");
                print_log_anyway
                    ("-->                  ReferenceType.methodsByName_ss() method can NOT be checked!");
                break;
            }
            if (!debugee_signal.equals("ready2")) {
                print_log_anyway
                    ("##> methbyname_ss003: UNEXPECTED debugee's signal (not \"ready2\") - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            else {
                print_log_on_verbose("--> methbyname_ss003: debugee's \"ready2\" signal recieved!");
            }
            print_log_on_verbose
                ("--> methbyname_ss003: check that checked class has been unloaded realy...");
            ReferenceType refType2 = debugee.classByName(checked_class);
            if (refType2 == null) {
                print_log_on_verbose
                    ("--> methbyname_ss003: checked class has been unloaded realy: " + checked_class);
            }
            else {
                print_log_without_verbose
                    ("--> methbyname_ss003: check that checked class has been unloaded realy...");
                print_log_anyway
                    ("--> methbyname_ss003: checked class FOUND: " + checked_class
            ...
            print_log_anyway
                ("--> methbyname_ss003: check ReferenceType.methodsByName(...) method for unloaded class...");
            List methods_byname_list = null;
            try {
                methods_byname_list = refType.methodsByName("dummy_method_name", "dummy_method_signature");
            }
            catch (Exception expt) {
                if (expt instanceof com.sun.jdi.ObjectCollectedException) {
                    print_log_anyway
                        ("--> methbyname_ss003: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    print_log_anyway
                        ("##> methbyname_ss003: FAILED: unexpected Exception thrown - " + expt.toString());
                    print_log_anyway
                        ("##>                  expected Exception - com.sun.jdi.ObjectCollectedException");
                    methodsByName_ss_method_error = true;
                }
            }
            break;
        }
        int v_test_result = 0/*STATUS_PASSED*/;
        if ( class_not_found_error || methodsByName_ss_method_error ) {
            v_test_result = 2/*STATUS_FAILED*/;
        }

        print_log_on_verbose("--> methbyname_ss003: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            print_log_anyway
                ("##> methbyname_ss003: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> methbyname_ss003: expected Debugee's exit status - " + status);
        }

        return v_test_result;
    }
}  // end of methbyname_ss003 class
