...
public class visibfield003 extends Log {
    ...
    private final static String
        ...
        thisClassName = package_prefix + "visibfield003",
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new visibfield003().runThis(argv,out_stream);
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        out_stream.println("==> nsk/jdi/ReferenceType/visibleFields/visibfield003 test LOG:");
        ...
        print_log_on_verbose("--> visibfield003: visibfield003a debugee launched");
        debugee.resume();

        String debugee_signal = pipe.readln();
        if (debugee_signal == null) {
            out_stream.println
                ("##> visibfield003: UNEXPECTED debugee's signal (not \"ready0\") - " + debugee_signal);
        ...
        if (!debugee_signal.equals("ready0")) {
            ...
            print_log_on_verbose("--> visibfield003: debugee's \"ready0\" signal recieved!");
        }
        ...
        if (debugee_signal == null) {
            out_stream.println
                ("##> visibfield003: UNEXPECTED debugee's signal (not \"ready1\") - " + debugee_signal);
            return 2/*STATUS_FAILED*/;
        }
        if (!debugee_signal.equals("ready1")) {
            ...
            print_log_on_verbose("--> visibfield003: debugee's \"ready1\" signal recieved!");
        }
        ...
        while ( true ) {
            print_log_on_verbose
                ("--> visibfield003: getting ReferenceType object for loaded checked class...");
            ReferenceType refType = debugee.classByName(checked_class);
            if (refType == null) {
                print_log_without_verbose
                    ("--> visibfield003: getting ReferenceType object for loaded checked class...");
                out_stream.println("##> visibfield003: FAILED: Could NOT FIND checked class: " + checked_class);
                class_not_found_error = true;
                break;
            }
            else {
                print_log_on_verbose("--> visibfield003: checked class FOUND: " + checked_class);
            }
            print_log_on_verbose
                ("--> visibfield003: waiting for \"ready2\" or \"not_unloaded\" signal from debugee...");
            pipe.println("continue");
            debugee_signal = pipe.readln();
            if (debugee_signal == null) {
                out_stream.println
                    ("##> visibfield003: UNEXPECTED debugee's signal - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            if ( debugee_signal.equals("not_unloaded")) {
                out_stream.println
                    ("--> visibfield003: debugee's \"not_unloaded\" signal recieved!");
                print_log_without_verbose
                    ("-->                checked class may be NOT unloaded!");
                out_stream.println
                    ("-->                ReferenceType.visibleFields() method can NOT be checked!");
                break;
            }
            if (!debugee_signal.equals("ready2")) {
                out_stream.println
                    ("##> visibfield003: UNEXPECTED debugee's signal (not \"ready2\") - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            else {
                print_log_on_verbose("--> visibfield003: debugee's \"ready2\" signal recieved!");
            }
            print_log_on_verbose
                ("--> visibfield003: check that checked class has been unloaded realy...");
            ReferenceType refType2 = debugee.classByName(checked_class);
            if (refType2 == null) {
                print_log_on_verbose
                    ("--> visibfield003: checked class has been unloaded realy: " + checked_class);
            }
            else {
                print_log_without_verbose
                    ("--> visibfield003: check that checked class has been unloaded realy...");
                out_stream.println
                    ("--> visibfield003: checked class FOUND: " + checked_class
            ...
            out_stream.println
                ("--> visibfield003: check ReferenceType.visibleFields() method for unloaded class...");
            ...
            try {
                ...
                if (expt instanceof com.sun.jdi.ObjectCollectedException) {
                    out_stream.println
                        ("--> visibfield003: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    out_stream.println
                        ("##> visibfield003: FAILED: unexpected Exception thrown - " + expt.toString());
                    out_stream.println
        ...
        print_log_on_verbose("--> visibfield003: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            out_stream.println
                ("##> visibfield003: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> visibfield003: expected Debugee's exit status - " + status);
        }

        return v_test_result;
    }
}  // end of visibfield003 class
