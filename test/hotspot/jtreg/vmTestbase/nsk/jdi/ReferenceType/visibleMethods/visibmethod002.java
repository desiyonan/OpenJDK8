...
//        package_prefix = "",    //  for DEBUG without package
        thisClassName = package_prefix + "visibmethod002",
        debugeeName   = thisClassName + "a";

    /** Debugee's classes for check **/
    private final static String class_for_check = package_prefix + "visibmethod002aClassForCheck";

    private final static String classLoaderName = package_prefix + "visibmethod002aClassLoader";
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new visibmethod002().runThis(argv,out_stream);
        if ( v_test_result == 2/*STATUS_FAILED*/ ) {
            logHandler.complain("\n==> nsk/jdi/ReferenceType/visibleMethods/visibmethod002 test FAILED");
        }
        else {
            logHandler.display("\n==> nsk/jdi/ReferenceType/visibleMethods/visibmethod002 test PASSED");
        }
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        print_log_on_verbose("--> visibmethod002: visibmethod002a debugee launched");
        debugee.resume();

        String line = pipe.readln();
        if (line == null) {
            print_log_anyway
                ("##> visibmethod002: UNEXPECTED debugee's signal (not \"ready\") - " + line);
        ...
        if (!line.equals("ready")) {
            ...
            print_log_on_verbose("--> visibmethod002: debugee's \"ready\" signal recieved!");
        }

        print_log_on_verbose
            ("--> visibmethod002: check ReferenceType.visibleMethods() method for not prepared "
        ...
        while ( true ) {  // test body
            ...
            if (isPrep) {
                print_log_anyway
                        ("##> visibmethod002: FAILED: isPrepared() returns for " + class_for_check + " : " + isPrep);
                class_not_found_error = true;
                break;
            } else {
                print_log_on_verbose
                    ("--> visibmethod002: isPrepared() returns for " + class_for_check + " : " + isPrep);
            }
            ...
            try {
                ...
                print_log_anyway
                    ("##> visibmethod002: FAILED: NO any Exception thrown!");
                print_log_anyway
                ...
                if (expt instanceof com.sun.jdi.ClassNotPreparedException) {
                    print_log_on_verbose
                        ("--> visibmethod002: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    print_log_anyway
                        ("##> visibmethod002: FAILED: unexpected Exception thrown - " + expt.toString());
                    print_log_anyway
                        ("##>                 expected Exception - com.sun.jdi.ClassNotPreparedException");
                    visibleMethods_method_error = true;
                }
        ...
        print_log_on_verbose("--> visibmethod002: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            print_log_anyway
                ("##> visibmethod002: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> visibmethod002: expected Debugee's exit status - " + status);
        }
