 ...
 * <code>com.sun.jdi.ReferenceType.visibleMethods()</code>
...
public class visibmethod006 {
    ...
    public static int run(String argv[], PrintStream out) {
        return new visibmethod006().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "visibmethod006t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                try {
                    log.display((i+1) + ") Trying to get a list containing visible methods in the reference type \""
                        + testedType + "\"\tname " + testedType.name()
                        + " ...");
                    List meths = testedType.visibleMethods();
                    if (meths.size() == 0)
                        log.display("CHECK PASSED: returned list is empty as expected");
                    else {
                        log.complain("TEST FAILED: returned list containing visible methods in the reference type \""
                            + testedType + "\"\tname: " + testedType.name()
                    ...
                    log.complain("TEST FAILED: caught unexpected "
                        + ue + "\n\tinstead of an empty list of visible methods"
