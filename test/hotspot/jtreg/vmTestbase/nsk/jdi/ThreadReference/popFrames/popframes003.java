...
//            log2("       eventIterator = eventSet.eventIterator;");
    ...
    private void eventForCommunication()
                 throws JDITestRuntimeException {

        log2("eventForCommunication");
        getEventSet();

        if (eventIterator.nextEvent() instanceof MethodEntryEvent)
            return;

        throw new JDITestRuntimeException("** event IS NOT a MethodEntryEvent **");
    }
