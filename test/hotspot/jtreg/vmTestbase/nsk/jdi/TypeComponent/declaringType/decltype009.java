...
public class decltype009 {
    ...
    private final static String className = "decltype009";
    ...
    private static void check (String fieldName) {
        try {
            ...
            for (int i = 0; i < expectedEnumFieldsNames.length; i++) {
                 ...
                 if (foundField != null) {
                     if (foundField.declaringType().equals(checkedClass)) {
                         display("enum " + className + " has field " + expectedEnumFieldsNames[i]);
                         display("\t of expected type " + className);
                     } else {
                         complain("enum " + className + " has field " + expectedEnumFieldsNames[i]);
                         complain("\t of unexpected type " + foundField.declaringType().name());
                         exitStatus = Consts.TEST_FAILED;
                     }
                 } else {
