...
Exception(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thr,
    ...
    err = (*jvmti_env)->GetClassSignature(jvmti_env, cls, &ex.name, &generic);
    ...
    err = (*jvmti_env)->GetClassSignature(jvmti_env, cls, &ex.t_cls, &generic);
    if (err != JVMTI_ERROR_NONE) {
    ...
    ex.t_loc = location;
    err = (*jvmti_env)->GetMethodDeclaringClass(jvmti_env, catch_method, &cls);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetMethodDeclaringClass#c) unexpected error: %s (%d)\n",
               TranslateError(err), err);
        result = STATUS_FAILED;
    }
    err = (*jvmti_env)->GetClassSignature(jvmti_env, cls, &ex.c_cls, &generic);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetClassSignature#c) unexpected error: %s (%d)\n",
               TranslateError(err), err);
        result = STATUS_FAILED;
    }
    err = (*jvmti_env)->GetMethodName(jvmti_env, catch_method,
...
Java_nsk_jvmti_Exception_exception001_check(JNIEnv *env, jclass cls) {
    ...
    if ((err = ((*jvmti)->GetCurrentThread(jvmti, &thread))) != JVMTI_ERROR_NONE) {
