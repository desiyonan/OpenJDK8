...
Java_nsk_jvmti_GetBytecodes_bytecodes002_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetBytecodes(jvmti, NULL, &bytecodeCount, &bytecodes);
    ...
    err = (*jvmti)->GetBytecodes(jvmti, mid, NULL, &bytecodes);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY && !caps.can_get_bytecodes) {
        /* It is OK */
    ...
    err = (*jvmti)->GetBytecodes(jvmti, mid, &bytecodeCount, NULL);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY && !caps.can_get_bytecodes) {
        /* It is OK */
    } else if (err != JVMTI_ERROR_NULL_POINTER) {
        printf("Error expected: JVMTI_ERROR_NULL_POINTER,\n");
    ...
    err = (*jvmti)->GetBytecodes(jvmti, mid, &bytecodeCount, &bytecodes);
