...
Java_nsk_jvmti_GetClassLoaderClasses_clsldrclss001_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetClassLoader(jvmti, cls, &classloader);
    ...
    err = (*jvmti)->GetClassLoaderClasses(jvmti, NULL, &classCount, &classes);
    ...
    err = (*jvmti)->GetClassLoaderClasses(jvmti, classloader, NULL, &classes);
    if (err != JVMTI_ERROR_NULL_POINTER) {
    ...
    err = (*jvmti)->GetClassLoaderClasses(jvmti, classloader, &classCount, NULL);
    if (err != JVMTI_ERROR_NULL_POINTER) {
        printf("Error expected: JVMTI_ERROR_NULL_POINTER,\n");
        printf("\tactual: %s (%d)\n", TranslateError(err), err);
        result = STATUS_FAILED;
