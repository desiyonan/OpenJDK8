...
Java_nsk_jvmti_GetLineNumberTable_linetab002_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetLineNumberTable(jvmti, NULL, &entryCount, &table);
    ...
    err = (*jvmti)->GetLineNumberTable(jvmti, mid, NULL, &table);
    ...
    err = (*jvmti)->GetLineNumberTable(jvmti, mid, &entryCount, NULL);
    if (err != JVMTI_ERROR_NULL_POINTER) {
