...
Java_nsk_jvmti_GetLocalVariableTable_localtab002_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetLocalVariableTable(jvmti, NULL, &entryCount, &table);
    ...
    err = (*jvmti)->GetLocalVariableTable(jvmti, mid, NULL, &table);
    ...
    err = (*jvmti)->GetLocalVariableTable(jvmti, mid, &entryCount, NULL);
    if (err != JVMTI_ERROR_NULL_POINTER) {
        printf("Error expected: JVMTI_ERROR_NULL_POINTER,\n");
        printf("\tactual: %s (%d)\n", TranslateError(err), err);
        result = STATUS_FAILED;
