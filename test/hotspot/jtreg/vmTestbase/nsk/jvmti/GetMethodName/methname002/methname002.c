...
Java_nsk_jvmti_GetMethodName_methname002_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetMethodName(jvmti, NULL, &name, &sig, &generic);
    ...
    err = (*jvmti)->GetMethodName(jvmti, mid, NULL, &sig, &generic);
    ...
    err = (*jvmti)->GetMethodName(jvmti, mid, &name, NULL, &generic);
    if (err != JVMTI_ERROR_NONE) {
    ...
    err = (*jvmti)->GetMethodName(jvmti, mid, &name, &sig, NULL);
    if (err != JVMTI_ERROR_NONE) {
        printf("(signaturePtr) unexpected error: %s (%d)\n",
               TranslateError(err), err);
        result = STATUS_FAILED;
