...
Java_nsk_jvmti_GetOwnedMonitorInfo_ownmoninf003_check(JNIEnv *env,
    ...
    err = (*jvmti)->GetOwnedMonitorInfo(jvmti, cls,
    ...
    err = (*jvmti)->GetOwnedMonitorInfo(jvmti, thr, NULL, &owned_monitors);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
    ...
    err = (*jvmti)->GetOwnedMonitorInfo(jvmti, thr, &owned_monitor_count, NULL);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
            !caps.can_get_monitor_info) {
        /* Ok, it's expected */
    } else if (err != JVMTI_ERROR_NULL_POINTER) {
