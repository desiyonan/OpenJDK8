...
jint Agent_Initialize(JavaVM *jvm, char *options, void *reserved) {
    ...
    res = (*jvmti)->GetPotentialCapabilities(jvmti, &caps);
    ...
    res = (*jvmti)->AddCapabilities(jvmti, &caps);
    if (res != JVMTI_ERROR_NONE) {
    ...
    res = (*jvmti)->GetCapabilities(jvmti, &caps);
    if (res != JVMTI_ERROR_NONE) {
...
Java_nsk_jvmti_GetSourceFileName_getsrcfn004_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetSourceFileName(jvmti, NULL, &name);
    ...
    err = (*jvmti)->GetSourceFileName(jvmti, cls, NULL);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY && caps.can_get_source_file_name) {
