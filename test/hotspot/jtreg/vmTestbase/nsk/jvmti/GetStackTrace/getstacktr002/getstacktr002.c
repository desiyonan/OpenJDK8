...
Java_nsk_jvmti_GetStackTrace_getstacktr002_check(JNIEnv *env, jclass cls, jthread thread) {
    ...
    err = (*jvmti)->GetStackTrace(jvmti, cls, 0, 1, &frame, &count);
    ...
    err = (*jvmti)->GetStackTrace(jvmti, thread, 0, -1, &frame, &count);
    ...
    err = (*jvmti)->GetStackTrace(jvmti, thread, 0, 1, NULL, &count);
    ...
    err = (*jvmti)->GetStackTrace(jvmti, thread, 0, 1, &frame, NULL);
    if (err != JVMTI_ERROR_NULL_POINTER) {
