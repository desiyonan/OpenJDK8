...
Java_nsk_jvmti_GetStackTrace_getstacktr003_check(JNIEnv *env, jclass cls, jthread thread) {
    ...
    err = (*jvmti)->GetStackTrace(jvmti, thread,
    ...
    for (i = 0; i < count; i++) {
        ...
        err = (*jvmti)->GetMethodDeclaringClass(jvmti, f[count-1-i].method,
            &callerClass);
        if (err != JVMTI_ERROR_NONE) {
        ...
        err = (*jvmti)->GetClassSignature(jvmti, callerClass,
            &sigClass, &generic);
        if (err != JVMTI_ERROR_NONE) {
        ...
        err = (*jvmti)->GetMethodName(jvmti, f[count-1-i].method,
            &name, &sig, &generic);
        if (err != JVMTI_ERROR_NONE) {
