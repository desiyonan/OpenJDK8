...
void check(jvmtiEnv *jvmti_env, jthread thr, int offset, char *note) {
    jvmtiError err;
...
void JNICALL Breakpoint(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    err = (*jvmti)->ClearBreakpoint(jvmti, mid_checkPoint, 0);
    if (err != JVMTI_ERROR_NONE) {
...
Java_nsk_jvmti_GetStackTrace_getstacktr008_getReady(JNIEnv *env, jclass cls,
    ...
    err = (*jvmti)->SetBreakpoint(jvmti, mid_checkPoint, 0);
    if (err != JVMTI_ERROR_NONE) {
