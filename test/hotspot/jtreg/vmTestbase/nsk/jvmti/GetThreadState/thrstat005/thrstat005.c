...
void reportError(const char * szErr, int res) {
#ifndef STANDALONE
...
jint Agent_Initialize(JavaVM *jvm, char *options, void *reserved) {
    ...
    res = (*g_ppJvmtiEnv)->CreateRawMonitor(g_ppJvmtiEnv, "beast", &g_waitMon);
    if ( res != JVMTI_ERROR_NONE ) {
        reportError("Agent_OnLoad: error creating raw monitor", res);
        return JNI_ERR;
    }
...
Java_nsk_jvmti_GetThreadState_thrstat005_setWaitTime(JNIEnv * pEnv, jclass class, jint waitTime) {
    g_waitTime = waitTime;
}

JNIEXPORT jboolean JNICALL
Java_nsk_jvmti_GetThreadState_thrstat005_checkThreadState(JNIEnv * pEnv, jclass class, jthread thread, jint stateIdx) {
    ...
    do {
        int res = (*g_ppJvmtiEnv)->GetThreadState(g_ppJvmtiEnv, thread, &thrState);
        if (res != JVMTI_ERROR_NONE) {
        ...
        fflush(stdout);
        if ( (res = (*g_ppJvmtiEnv)->RawMonitorEnter(g_ppJvmtiEnv, g_waitMon)) != JVMTI_ERROR_NONE
                || (res = (*g_ppJvmtiEnv)->RawMonitorWait(g_ppJvmtiEnv, g_waitMon, waitTime)) != JVMTI_ERROR_NONE
                || (res = (*g_ppJvmtiEnv)->RawMonitorExit(g_ppJvmtiEnv, g_waitMon)) != JVMTI_ERROR_NONE ) {
            reportError("GetThreadState: unexpected error", res);
            return JNI_FALSE;
        }
