...
Java_nsk_jvmti_IsFieldSynthetic_isfldsin002_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->IsFieldSynthetic(jvmti, NULL, fid, &isSynthetic);
    ...
    err = (*jvmti)->IsFieldSynthetic(jvmti, cls, NULL, &isSynthetic);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
            !caps.can_get_synthetic_attribute) {
        /* It is OK */
    ...
    err = (*jvmti)->IsFieldSynthetic(jvmti, cls, fid, NULL);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
            !caps.can_get_synthetic_attribute) {
        /* It is OK */
