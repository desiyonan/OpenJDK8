...
Java_nsk_jvmti_PopFrame_popframe011_doPopFrame(JNIEnv *env,
    ...
    if (t_case != 6 && t_case != 7) {
         ...
         * should not happen.
         */
        popframe_err = (*jvmti)->SetEventNotificationMode(jvmti, JVMTI_ENABLE,
...
/* NULL pointer to the thread */
    case 0:
        ...
        popframe_err = ((*jvmti)->PopFrame(jvmti, NULL)); /* explode the bomb */
...
/* invalid thread */
    case 2:
        set_watch_ev(1); /* watch JVMTI events */
        popframe_err = ((*jvmti)->PopFrame(jvmti, cls)); /* explode the bomb */
...
/* non suspended thread */
    case 4:
        set_watch_ev(1); /* watch JVMTI events */
        popframe_err = ((*jvmti)->PopFrame(jvmti, frameThr)); /* explode the bomb */
        set_watch_ev(0); /* ignore again JVMTI events */
...
/* non suspended and exiting thread */
    ...
    if (t_case != 6 && t_case != 7) {
         ...
         * should not happen.
         */
        popframe_err = (*jvmti)->SetEventNotificationMode(jvmti, JVMTI_DISABLE,
            JVMTI_EVENT_METHOD_EXIT, frameThr);
        if (popframe_err != JVMTI_ERROR_NONE) {
