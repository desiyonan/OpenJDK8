...
increment_thread(jvmtiEnv* jvmti, JNIEnv* jni, void *unused) {
    ...
    for (i = 0; i < INCREMENT_LIMIT; i++) {
        err = (*jvmti)->RawMonitorEnter(jvmti, monitor);
...
Java_nsk_jvmti_RawMonitorEnter_rawmonenter004_check(JNIEnv *env, jclass cls, jarray threads, jint wtime) {
    jvmtiError err;
    ...
    err = (*jvmti)->CreateRawMonitor(jvmti, "wait lock", &wait_lock);
    if (err != JVMTI_ERROR_NONE) {
    ...
    for (i = 0; i < threads_limit; i++) {
        thr = JNI_ENV_PTR(env)->GetObjectArrayElement(JNI_ENV_ARG(env,
            threads), i);
        err = (*jvmti)->RunAgentThread(jvmti, thr, increment_thread, NULL,
    ...
    for (i = 0; i < wait_time/WAIT_STEP &&
        ...
        err = (*jvmti)->RawMonitorWait(jvmti, wait_lock, (jlong)WAIT_STEP);
        if (err != JVMTI_ERROR_NONE) {
