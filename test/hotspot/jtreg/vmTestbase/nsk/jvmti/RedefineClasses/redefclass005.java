 ...
 * The test creates an instance of tested class <code>redefclass005r</code>.
 * Then the test makes classfile redifinition of the loaded class
 * <code>redefclass005r</code>.<br>
 * Bytes defining new version of the class <code>redefclass005r</code>
 * are taken from the same classfile, i.e. actually the new version
 * is identical with the old version.
 */
public class redefclass005 {
    ...
    static {
        try {
            System.loadLibrary("redefclass005");
        } catch (UnsatisfiedLinkError e) {
            System.err.println("Could not load redefclass005 library");
    ...
    public static int run(String argv[], PrintStream out) {
        return new redefclass005().runIt(argv, out);
    }
    ...
    private int runIt(String argv[], PrintStream out) {
        ...
        redefclass005r redefClsObj = new redefclass005r();
        if (redefClsObj.checkIt(out, DEBUG_MODE) == 19) {
            if (DEBUG_MODE)
                out.println("Successfully check the class redefclass005r");
        } else {
            out.println("TEST: failed to check the class redefclass005r");
            return FAILED;
        }

// try to redefine class redefclass005r
        String fileName =
            redefclass005r.class.getName().replace('.', File.separatorChar) +
            ".class";
        if (DEBUG_MODE)
            out.println("Trying to redefine class from the file: " + fileName);
        try {
            ClassLoader cl = redefclass005.class.getClassLoader();
