...
void setWatch(JNIEnv *env, jint ind) {
    ...
    cls = JNI_ENV_PTR(env)->FindClass(JNI_ENV_ARG(env, fields[ind].class));
    ...
    err = (*jvmti)->SetFieldModificationWatch(jvmti, cls, fields[ind].fid);
...
void JNICALL FieldModification(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    jint fld_ind = 0;
    size_t len = 0;
    jvmtiError err = (*jvmti_env)->GetFieldName(jvmti_env, field_klass, field,
                                                &fld_name, NULL, NULL);
    ...
    (*jvmti)->Deallocate(jvmti_env, (unsigned char*) fld_name);
}
