...
JNIEXPORT void JNICALL doRedefineInNativeThread(jvmtiEnv * jvmti,
  ...
  testClass = (*jni)->FindClass(jni, MAIN_CLASS);
  ...
  cla = (*jni)->FindClass(jni, SEARCH_NAME);
...
Java_nsk_jvmti_scenarios_hotswap_HS103_hs103t002_hs103t002_startAgentThread(JNIEnv * jni, jclass cls) {
  ...
  name = (*jni)->NewStringUTF(jni, threadName);
  clas = (*jni)->FindClass(jni, "java/lang/Thread");
  ...
  thread = (jthread) (*jni)->NewObject(jni, clas,method,name);
  testAgentThread = (*jni)->NewGlobalRef(jni, thread);
  err = JVMTI_ERROR_NONE;
  err = (*jvmti)->RunAgentThread(jvmti, testAgentThread, &doRedefineInNativeThread, NULL,
