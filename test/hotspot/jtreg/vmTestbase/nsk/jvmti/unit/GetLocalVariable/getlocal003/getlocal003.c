...
void JNICALL MethodExit(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    if (mid == method) {
        ...
        err = (*jvmti_env)->GetFrameLocation(jvmti_env, thr, 0,
        ...
        for (i = 0; i < entryCount; i++) {
            ...
            err = (*jvmti)->GetLocalFloat(jvmti, thr, 0, table[i].slot, &floatVal);
            ...
            err = (*jvmti)->GetLocalObject(jvmti, thr, 0, table[i].slot, &obj);
...
Java_nsk_jvmti_unit_GetLocalVariable_getlocal003_checkLoc(JNIEnv *env,
    ...
    for (i = 0; i < entryCount; i++) {
        ...
        err = (*jvmti)->GetLocalInt(jvmti, thr, 1, table[i].slot, &locVar);
