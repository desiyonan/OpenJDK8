...
checkClassMethods(jclass klass) {
    ...
    err = (*jvmti)->GetClassMethods(jvmti, klass, &meth_cnt, &methods_ptr);
    if (err != JVMTI_ERROR_NONE) {
    ...
    for (i = 0; i < meth_cnt; i++) {
        jint mods;
        mid = methods_ptr[i];
        err = (*jvmti)->GetMethodName(jvmti, mid, &name_ptr,
                                    &sign_ptr, (char **) NULL);
        if (err != JVMTI_ERROR_NONE) {
        ...
        err = (*jvmti)->IsMethodSynthetic(jvmti, mid, &isSynthetic);
        if (err != JVMTI_ERROR_NONE) {
        ...
        err = (*jvmti)->GetMethodModifiers(jvmti, mid, &mods);
        if (err != JVMTI_ERROR_NONE) {
...
checkClassFields(jclass klass) {
    ...
    err = (*jvmti)->GetClassFields(jvmti, klass, &fld_cnt, &fields_ptr);
    if (err != JVMTI_ERROR_NONE) {
    ...
    for (i = 0; i < fld_cnt; i++) {
        jint mods;
        fid = fields_ptr[i];
        err = (*jvmti)->GetFieldName(jvmti, klass, fid, &name_ptr,
                                    &sign_ptr, (char **) NULL);
        if (err != JVMTI_ERROR_NONE) {
        ...
        err = (*jvmti)->IsFieldSynthetic(jvmti, klass, fid, &isSynthetic);
        if (err != JVMTI_ERROR_NONE) {
        ...
        err = (*jvmti)->GetFieldModifiers(jvmti, klass, fid, &mods);
        if (err != JVMTI_ERROR_NONE) {
...
Java_nsk_jvmti_unit_IsSynthetic_issynth001_check(JNIEnv *env,
    ...
    err = (*jvmti)->GetClassSignature(jvmti, cls, &class_sign, (char **) NULL);
    if (err != JVMTI_ERROR_NONE) {
    ...
    */
    ...
    err = (*jvmti)->GetClassSignature(jvmti, klass, &class_sign, (char **) NULL);
    if (err != JVMTI_ERROR_NONE) {
        printf("GetSourceFileName unexpected error: %s (%d)\n",
               TranslateError(err), err);
        result = STATUS_FAILED;
