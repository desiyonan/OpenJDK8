...
Java_nsk_jvmti_unit_MethodBind_JvmtiTest_CreateRawMonitor(JNIEnv * env, jclass class, jint i) {
    jvmtiError ret;
    char sz[128];
...
print_method_name(jmethodID mid) {
    ...
    char *signature;
    char *clname = "unknown";
    ret = JVMTI_ENV_PTR(jvmti)->GetMethodDeclaringClass(JVMTI_ENV_ARG1(jvmti), mid, &class);
    if (ret != JVMTI_ERROR_NONE) {
    ...
    ret = JVMTI_ENV_PTR(jvmti)->GetMethodName(JVMTI_ENV_ARG1(jvmti), mid, &mname, &signature, NULL);
    if (ret != JVMTI_ERROR_NONE) {
