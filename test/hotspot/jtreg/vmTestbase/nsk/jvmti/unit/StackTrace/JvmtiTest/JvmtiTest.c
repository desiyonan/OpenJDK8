...
Java_nsk_jvmti_unit_StackTrace_JvmtiTest_CreateRawMonitor(JNIEnv * env, jclass class, jint i) {
    jvmtiError ret;
    char sz[128];
...
Java_nsk_jvmti_unit_StackTrace_JvmtiTest_GetStackTrace(JNIEnv * env, jclass cls, jobject thr) {
    ...
    ret = JVMTI_ENV_PTR(jvmti)->Allocate(JVMTI_ENV_ARG1(jvmti), sizeof(jvmtiFrameInfo) * max_count, (unsigned char**)&stack_buffer);
    if (ret != JVMTI_ERROR_NONE) {
        printf("Error: Allocate failed with  %d \n", ret);
        iGlobalStatus = 2;
    }

    ret = JVMTI_ENV_PTR(jvmti)->Allocate(JVMTI_ENV_ARG1(jvmti), sizeof(jvmtiFrameInfo) * max_count, (unsigned char**)&compare_buffer);
    if (ret != JVMTI_ERROR_NONE) {
        printf("Error: Allocate failed with  %d \n", ret);
        iGlobalStatus = 2;
    }
    ...
    ret = JVMTI_ENV_PTR(jvmti)->SuspendThread(JVMTI_ENV_ARG1(jvmti), thr);
    if (ret != JVMTI_ERROR_NONE) {
        printf("Error: SuspendThread failed with  %d \n", ret);
        iGlobalStatus = 2;
    }

    ret = JVMTI_ENV_PTR(jvmti)->GetStackTrace(JVMTI_ENV_ARG1(jvmti), thr, 0, max_count , stack_buffer, &count);
    if (ret != JVMTI_ERROR_NONE) {
    ...
    for (i = 1; i < count; ++i) {
      int res_count = 48343;
      ret = JVMTI_ENV_PTR(jvmti)->GetStackTrace(JVMTI_ENV_ARG1(jvmti), thr, i, max_count , compare_buffer, &res_count);
      if (ret != JVMTI_ERROR_NONE) {
    ...
    ret = JVMTI_ENV_PTR(jvmti)->ResumeThread(JVMTI_ENV_ARG1(jvmti), thr);
    if (ret != JVMTI_ERROR_NONE) {
    ...
    for (i = 0; i < count; i++) {
        ...
        ret = JVMTI_ENV_PTR(jvmti)->GetClassSignature(JVMTI_ENV_ARG1(jvmti), class, &clname, NULL);
        if (ret != JVMTI_ERROR_NONE) {
        ...
        ret = JVMTI_ENV_PTR(jvmti)->GetMethodName(JVMTI_ENV_ARG1(jvmti), stack_buffer[i].method, &mname, &signature, NULL);
        if (ret != JVMTI_ERROR_NONE) {
