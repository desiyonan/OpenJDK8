  ...
  (JNIEnv *env, jclass cls)
{
    ...
    err = (*jvmti)->GetExtensionFunctions(jvmti, &count, &ext_funcs);
  ...
  (JNIEnv *env, jclass cls, jboolean enable)
{
    ...
    err = (*jvmti)->GetExtensionEvents(jvmti, &count, &ext_events);
    if (err != JVMTI_ERROR_NONE) {
    ...
    for (i=0; i<count; i++) {
        if (strcmp(ext_events[i].id, (char*)"com.sun.hotspot.events.ClassUnload") == 0) {
            ...
            err = (*jvmti)->SetExtensionEventCallback(jvmti, ext_events[i].extension_event_index,
