...
public class JvmtiTest {
    ...
    final static String BOOT_CLASS =
        "nsk.jvmti.unit.functions.AddToBootstrapClassLoaderSearch.Boot";

    static {
        try {
            System.loadLibrary("AddToBootstrapClassLoaderSearch");
        } catch (UnsatisfiedLinkError ule) {
            System.err.println("Could not load AddToBootstrapClassLoaderSearch library");
    ...
    public static int run(String args[], PrintStream out) {
        // Boot b = new Boot();
        try {
            Object b = Class.forName(BOOT_CLASS).newInstance();
        } catch (Exception e) {
