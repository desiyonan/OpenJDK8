...
void JNICALL vmStart(jvmtiEnv *jvmti_env, JNIEnv *env) {
    jvmtiError res;
    res = JVMTI_ENV_PTR(jvmti)->GetCurrentThread(JVMTI_ENV_ARG1(jvmti_env), &main_thread);
...
void JNICALL vmInit(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thread) {
    ...
    res = JVMTI_ENV_PTR(jvmti)->RawMonitorExit(JVMTI_ENV_ARG1(jvmti_env),access_lock);
...
void JNICALL classFileLoadHookEvent(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    res = JVMTI_ENV_PTR(jvmti)->GetPhase(JVMTI_ENV_ARG1(jvmti_env), &phase);
    JVMTI_ERROR_CHECK_VOID(" JVMTI GetPhase returned error", res);
    if (phase != JVMTI_PHASE_START) {
        return; /* only the start phase is tested */
    }
    res = JVMTI_ENV_PTR(jvmti)->GetCurrentThread(JVMTI_ENV_ARG1(jvmti_env), &thread);
    JVMTI_ERROR_CHECK_VOID(" JVMTI GetCurrentThread returned error", res);
    ...
    if (process_once) {
        ...
        res = JVMTI_ENV_PTR(jvmti)->RawMonitorExit(JVMTI_ENV_ARG1(jvmti_env),access_lock_not_entered);
        ...
        res = JVMTI_ENV_PTR(jvmti)->RawMonitorEnter(JVMTI_ENV_ARG1(jvmti),access_lock);
...
jint Agent_Initialize(JavaVM * jvm, char *options, void *reserved) {
    ...
    res = JVMTI_ENV_PTR(jvmti)->RawMonitorExit(JVMTI_ENV_ARG1(jvmti),access_lock);
    JVMTI_ERROR_CHECK("Raw monitor exit returned error", res);

    /* test not entered raw monitor */
    res = JVMTI_ENV_PTR(jvmti)->RawMonitorExit(JVMTI_ENV_ARG1(jvmti),access_lock_not_entered);
    ...
    res = JVMTI_ENV_PTR(jvmti)->DestroyRawMonitor(JVMTI_ENV_ARG1(jvmti),access_lock);
     ...
     */
    res = JVMTI_ENV_PTR(jvmti)->RawMonitorEnter(JVMTI_ENV_ARG1(jvmti),jraw_monitor[0]);
    JVMTI_ERROR_CHECK("Raw monitor enter returned error", res);
    res = JVMTI_ENV_PTR(jvmti)->RawMonitorEnter(JVMTI_ENV_ARG1(jvmti),jraw_monitor[0]);
    JVMTI_ERROR_CHECK("Raw monitor enter returned error", res);
    res = JVMTI_ENV_PTR(jvmti)->RawMonitorExit(JVMTI_ENV_ARG1(jvmti),jraw_monitor[0]);
    JVMTI_ERROR_CHECK("Raw monitor exit returned error", res);
...
Java_nsk_jvmti_unit_functions_rawmonitor_CreateRawMonitor(JNIEnv * env, jclass class, jint i) {
    jvmtiError ret;
    char sz[128];
