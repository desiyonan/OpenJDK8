...
intptr_t get_env_local() {
  jvmtiError res;
  void *val;
  res = JVMTI_ENV_PTR(jvmti)->GetEnvironmentLocalStorage(jvmti, &val);
...
void set_env_local(intptr_t x) {
  jvmtiError res;
  void *val = (void*)x;
  res = JVMTI_ENV_PTR(jvmti)->SetEnvironmentLocalStorage(jvmti, val);
...
intptr_t get_thread_local(jthread thread) {
  jvmtiError res;
  void *val;
  res = JVMTI_ENV_PTR(jvmti)->GetThreadLocalStorage(jvmti, thread, &val);
...
void set_thread_local(jthread thread, intptr_t x) {
  jvmtiError res;
  void *val = (void*)x;
  res = JVMTI_ENV_PTR(jvmti)->SetThreadLocalStorage(jvmti, thread, val);
  JVMTI_ERROR_CHECK_VOID("SetThreadLocalStorage returned error", res);
}

void check_val(intptr_t x, intptr_t y, char *msg) {
  if (x != y) {
